...
function createFreshNewDraft(userName?: string): VaultDraft {
  ...
  return {
    ...
    description:
      'บรรยากาศยามบ่าย ณ เรียวกังออนเซ็นกลางทิวเขาอันสงบเงียบ ควรจะเป็นช่วงเวลาแห่งการพักผ่อน แต่ชมรมพฤกษศาสตร์กลับได้รับภารกิจด่วนในการเก็บกู้สมุนไพรหายาก',
    status: 'draft',
    isPinned: false,
    ...
    starting_state: {
      time: 'ยามบ่าย',
      weather: 'แดดยามบ่ายเงียบสงัด ก่อนเมฆดำทะมึนจะกลืนทิวเขา',
    ...
    worldVisual: 'เขียวมรกตพฤกษศาสตร์ตัดแดงคาร์ไมน์พิษร้อน',
    worldSound: 'ไอหมอกกำมะถันออนเซ็น ปะทะ ลมหนาวชื้นกลางป่าสน',
  };
}
    ...
    async function loadDraftContext() {
      ...
      try {
        ...
        if (isMounted && history && history.length > 0) {
          ...
          const worldTitle = target?.thai_name || target?.worldTitle || 'โลกและสถานการณ์';
          setMessages([
            {
              id: `muse-${Date.now()}`,
              sender: 'muse',
